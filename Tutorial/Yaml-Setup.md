...
### A .yaml file is a configuration file that allows you to select all your variables for the main shell script, collapsing_lclust.sh. 

### YAML Variables 

USER_VARIABLE:
The case_group variable is what you want the project to be named. All files will include the case_group name. The case_path is the path to your case list.

COLLAPSING_VARIABLE:
The resolution variable is the resolution used for clustering and is used for the generation of clusters and identifying files of that resolution. min_sample is the minimum size number of cases/controls of clusters. max_ratio is the desired case/control ratio. models_to_exclude lists models to exclude for gene based CMH. 

ADDITIONAL_PARAMETERS:

PERMUTATION_VARIABLE: 
These are all variables used for CMH permutations. permstart states what should be the first permutation. permend states what should be the last permutation. cores states the number of cores to be used on the machine. model lists the models to permute over.

MODEL_VARIABLE:
Only thing needed to change is the model_num. model_num lists models to run, place model num/range that you would like to run in quotes. You can also make your own models by creating another model number (ie. "model_16: ") with different ATAV options. Learn what each of the different options for the models mean <a href="/Tutorial/selecting_models_options.md">here</a>.
...
Here is an example of what your YAML file should look like. Remember to set each variable to reflect the type of analysis that you want to run:
