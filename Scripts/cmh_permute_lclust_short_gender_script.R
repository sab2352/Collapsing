...
GetOptList <- function() {
  ...
  option.list <- list(
    ...
    make_option(c("-g", "--gender"), action="store", 
                default="male", dest="gender", 
                help="gender, [default %default]"),
...
setwd(opt$project)
source("../Scripts/cmh_permute_lclust_short_gender_functions.R")
...
nclust <- (cl_sizes %>% filter(Gender == gender & case >= min_sample & control >= min_sample))$cluster
...
getCMHresP(model, dir, gender, pc_path, nclust, nperm, cores)
