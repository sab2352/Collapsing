...
doPermutations <- function(matrix.data, perm, gender) {
    ...
    saveRDS(cmhs.tidy, paste0(permut_path, dir, model, "_CMH_perm_res_", resolution, "_genderstrat_min_sample_", min_sample, "_perm_", perm, ".RDS"))
}

getCMHresP <- function(model, dir, genders, pc_path, nclusts, nperm = c(1), cores = 1) {
    
    logfile <- paste0("Log/messagesperm_", dir, model, "_genderstrat_", nperm[1], "_", Sys.Date(), ".log")
    con <- file(logfile, "w")
    sink(con, append = TRUE, type = "message")
    print(model)
    
    matrix.data <- flatten(lapply(genders, function(x) lapply(nclusts[[x]], function(y) readMatrix(model, dir, x, pc_path, y))))
    names(matrix.data) <- unlist(lapply(genders, function(x) paste(x, nclusts[[x]], sep = "_")))
